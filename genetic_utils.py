...
def crossOver(chromosome1, chromosome2, chance=0.25):
    ...
    for i in xrange(len(chromosome1)):
        ...
        tmp1 = (((p1 + r1) / 2.0, (q1 + s1) / 2.0), ((p2 + r2) / 2.0,
                                                     (q2 + s2) / 2.0), X)
        tmp2 = (((p1 + r1) / 2.0, (q1 + s1) / 2.0), ((p2 + r2) / 2.0,
                                                     (q2 + s2) / 2.0), Y)
...
def mutate(chromosome, source, target, rate=0.4):
    ...
    for i in xrange(len(chromosome)):
        ...
        mutation.append(tmp)
    if random() < (sqrt(sqrt(len(chromosome) - 1)) * fitness(
            multiFold(source, chromosome)[0][-1], target)**4) / 10.0:
        ...
        if fupto:
            ...
            spopul = firstGen(source, target, bfolds, 100,
                              len(chromosome) - fupto)
            for i in xrange(len(spopul)):
                spopul.append(mutate(spopul[i], src, target))
            for i in xrange(3):
                spopul, bestfitness, bestchromosome = newGeneration(
                    src, target, spopul)
            mutation = tuple(list(chromosome[:fupto]) + list(bestchromosome))
            print 'recursive: genetic-programming:: recursed at ', ` len(
                chromosome) `, ' at ', ` fupto `, ': initial-fitness', ` fitness(
                    multiFold(source, chromosome)[0][-1],
                    target) `, ' final-fitness', ` fitness(
                        multiFold(source, mutation)[0][-1], target) `
...
def newGeneration(source, target, population, verbose=False):
    ...
    while len(newpop) < len(population):
        ...
        if verbose:
            reqFitness = getPopFitness(source, target,
